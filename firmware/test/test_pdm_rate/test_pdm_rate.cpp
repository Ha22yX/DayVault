...
void test_divider_50_keeps_mic_in_normal_mode(void)
{
...
{
    TEST_ASSERT_EQUAL_UINT32(
        1600000u, PDM_DFSDM_SOURCE_HZ / PDM_PRODUCTION_CKOUT_DIVIDER);
    TEST_ASSERT_EQUAL_UINT32(
        16000u,
...
void test_production_clock_and_rate_contract(void)
{
    const uint32_t clock = pdm_clock_hz(
        PDM_DFSDM_SOURCE_HZ, PDM_PRODUCTION_CKOUT_DIVIDER);
    TEST_ASSERT_EQUAL_UINT32(1600000u, clock);
    TEST_ASSERT_EQUAL_UINT32(
        16000u,
        pdm_pcm_rate_hz(PDM_DFSDM_SOURCE_HZ, PDM_PRODUCTION_CKOUT_DIVIDER,
                        PDM_PRODUCTION_OSR));
...
{
    UNITY_BEGIN();
    RUN_TEST(test_divider_50_keeps_mic_in_normal_mode);
    RUN_TEST(test_production_rate_is_exact_opus_wideband_rate);
