...
| 单路 PDM 调试 | 已完成电气规划 | 当前快照把 PDM 数据接到 `PB1/DFSDM1_DATIN0` |
| 双麦克风立体声 | 必须改版 | 把 PDM 数据移到 `PB12/DFSDM1_DATIN1`，再验证成对 DFSDM 通道 |
| PCB 同步 | 阻塞 | USB 插入检测器件尚未从原理图同步到 PCB |
| PCB 布局 | 阻塞 | 仍需处理 DRC、地平面、电源布线、USB 布线和 SWD 调试入口 |
